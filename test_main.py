...
import imageio # used for read the image and when running test.py
import matplotlib.pyplot as plt # used only when running test.py
import os # used only when running test.py
import sys # used only when running test.py

######
# PLOT METHODS
######
def plot_spectrum_analysis(image,fourier_spectrum,case_index):
	...
	plt.subplot(222)
	plt.title('Spectrum')
	plt.imshow(np.abs(fourier_spectrum),cmap="gray")
	plt.subplot(223)
	plt.title('Shiftted Spectrum')
	plt.imshow(np.fft.fftshift(np.abs(fourier_spectrum)),cmap="gray")
	plt.subplot(224)
	plt.title('Log-shiftted spectrum')
	plt.imshow(np.log(1 + np.fft.fftshift(np.abs(fourier_spectrum))),cmap="gray")
	plt.axis('off')
	plt.savefig('plots/case_'+case_index+'_spectrum_analysis.png',bboxinches='tight')

def plot_frequency_analysis(fourier_spectrum,case_index):
	plt.figure(figsize=(8,8))
	width, height = fourier_spectrum.shape
	y = np.abs(fourier_spectrum.reshape((width*height)))
	plt.plot(y)
	plt.savefig('plots/case_'+case_index+'_frequency_analysis.png',bboxinches='tight')

def plot_image(image,case_index):
	plt.figure(figsize=(8,8))
	plt.imshow(np.abs(image),cmap="gray")
	plt.axis('off')
	plt.savefig('plots/case_'+case_index+'_output_image.png',bboxinches='tight')
...
if len(sys.argv) > 1:
	plot_spectrum_analysis(input_img,fourier_spectrum,sys.argv[1])

# 3. Getting the second peak
p2 = get_second_peak(fourier_spectrum)
if len(sys.argv) > 1:
	plot_frequency_analysis(fourier_spectrum,sys.argv[1])
